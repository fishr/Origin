...
void LCD_ChipSelect(FunctionalState NewState)
{
  if (NewState == DISABLE)
  {
    HAL_GPIO_ResetBits(LCD_NCS_GPIO_PORT, LCD_NCS_PIN); /* CS pin low: LCD disabled */
  }
  else
  {
    HAL_GPIO_SetBits(LCD_NCS_GPIO_PORT, LCD_NCS_PIN); /* CS pin high: LCD enabled */
  }
//  delay(100);
}
//
...
void LCD_WriteCommand(uint8_t LCD_Reg)
{
  ...
  LCD_CtrlLinesWrite(LCD_WRX_GPIO_PORT, LCD_WRX_PIN, GPIO_PIN_RESET);
  
  /* Reset LCD control line(/CS) and Send command */
  LCD_ChipSelect(DISABLE);
  HAL_SPI_Transmit(&hspi5, (uint8_t*) &LCD_Reg, 1, LCD_SPI_TIMEOUT_MAX);
  delay(100);
  LCD_ChipSelect(ENABLE);
  delay(50);
}
...
void LCD_WriteData(uint8_t value)
{
  ...
  LCD_CtrlLinesWrite(LCD_WRX_GPIO_PORT, LCD_WRX_PIN, GPIO_PIN_SET);
  
  /* Reset LCD control line(/CS) and Send data */  
  LCD_ChipSelect(DISABLE);
  HAL_SPI_Transmit(&hspi5, (uint8_t*) &value, 1, LCD_SPI_TIMEOUT_MAX);
  delay(100);
  LCD_ChipSelect(ENABLE);
  delay(50);
}
...
/**
  ...
  * @retval None
  */
void LCD_CtrlLinesWrite(GPIO_TypeDef* GPIOx, uint16_t CtrlPins, GPIO_PinState PinState)
{
  /* Set or Reset the control line */
  HAL_GPIO_WritePin(GPIOx, (uint16_t)CtrlPins, (GPIO_PinState)PinState);
}
